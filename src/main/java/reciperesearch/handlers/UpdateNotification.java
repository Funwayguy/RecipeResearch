...
public class UpdateNotification
{
	...
	{
		if(!RecipeResearch.proxy.isClient() || hasChecked)
		{
			return;
		}
		...
		if(RecipeResearch.VERSION == "RR_VER_" + "KEY")
		{
			event.player.addChatMessage(new ChatComponentText(EnumChatFormatting.RED + "THIS COPY OF " + RecipeResearch.NAME.toUpperCase() + " IS NOT FOR PUBLIC USE!"));
			return;
		}
		
		try
		{
			String[] data = getNotification("http://bit.ly/1CnWMZY", true);
			
			if(RR_Settings.hideUpdates)
			{
				return;
			}
			...
			int verStat = compareVersions(RecipeResearch.VERSION, version);
			
			if(verStat == -1)
			{
				event.player.addChatMessage(new ChatComponentText(EnumChatFormatting.RED + "Update " + version + " of " + RecipeResearch.NAME + " available!"));
			...
			{
				event.player.addChatMessage(new ChatComponentText(EnumChatFormatting.YELLOW + RecipeResearch.NAME + " " + RecipeResearch.VERSION + " is up to date"));
			} else if(verStat == 1)
			{
				event.player.addChatMessage(new ChatComponentText(EnumChatFormatting.RED + RecipeResearch.NAME + " " + RecipeResearch.VERSION + " is a debug build"));
			} else if(verStat == -2)
			{
				event.player.addChatMessage(new ChatComponentText(EnumChatFormatting.RED + "An error has occured while checking " + RecipeResearch.NAME + " version!"));
			}
